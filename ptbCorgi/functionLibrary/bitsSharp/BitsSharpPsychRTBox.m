...
%Given numberic handle don't try and open a new serial port just 
if isnumeric(deviceID)
    
    s = deviceID;
    port ='UnkownGivenHandleNotPort';
...
else %try and find/connect to portname
    ...
    while (trycount < 2) && (~deviceFound)
        ...
        for ic=1:nPorts
            ...
            %BITS sharp is unable to support <1msec ReceiveLatency so
            %removinig it from opening command.
...
end%Done opening/connecting/looking for device

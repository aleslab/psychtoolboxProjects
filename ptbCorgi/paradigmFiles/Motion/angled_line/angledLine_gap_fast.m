function [conditionInfo, expInfo] = angledLine_gap_fast(expInfo)

%paradigm for exp 9 (no integration experiment).

expInfo.paradigmName = 'angledLine_gap_fast';
...
for iGap = 1:length(temporalGap);
    ...
    for iSpeed = 1: length(section2velocity);
        ...
        %general
        conditionInfo(iCond).trialFun=@angledLineTrial; %This defines what function to call to draw the condition
        ...
        conditionInfo(iCond).angledLines = true;
        conditionInfo(iCond).lineLength = 4; %4 deg
        conditionInfo(iCond).lineAngle = 45; %45 degrees
        ...
        conditionInfo(iCond).startPos = 0; %the start position of the line on the screen in degrees of visual angle.

function [conditionInfo, expInfo] = movingDots_continuous_alternative(expInfo)

%paradigm for exp 9 (lines vs. dots experiment).

expInfo.paradigmName = 'movingDots_continuous_alternative';
...
iCond = 1; %defining here initially to prevent overwriting
    ...
    for iSpeed = 1: length(section2velocity);
        ...
        conditionInfo(iCond).conditionType = 'continuousAlternative'; %so the moving lines move one after the other

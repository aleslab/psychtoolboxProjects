function [conditionInfo, expInfo] = lateralLine_speedchange_balanced_max_match(expInfo)

%paradigm for speed change experiment (exp 5). Distance and duration are
%balanced, and speeds are matched for the maximum speeds of the unbalanced
%conditions.

expInfo.paradigmName = 'lateralLine_speedchange_balanced_max_match';
...
%velocities
section1velocity = [16 15 14 13 12 11 10]; %velocities to use in deg/s;
...
for iSpeed = 1: length(section1velocity);
    ...
    conditionInfo(iCond).velocityDegPerSecSection2 = 32 - section1velocity(iSpeed); %velocity of section 2 in deg/s
    ...
    nullCondition.velocityDegPerSecSection1 = 16;  %then always setting the velocity to be the standard
    nullCondition.velocityDegPerSecSection2 = 16; %in both sections

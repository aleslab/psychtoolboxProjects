function [conditionInfo, expInfo] = lateralLine_longDuration(expInfo)

%paradigm for exp 6 (experiment investigating the role of distance and 
%duration in speed discrimination).

expInfo.paradigmName = 'lateralLine_longDuration';
...
%for iGap = 1:length(spatialGap);
    ...
    for iSpeed = 1: length(section2velocity);
        ...
        conditionInfo(iCond).label = ['long_dur_second_speed_' num2str(section2velocity(iSpeed))];
        ...
        conditionInfo(iCond).stimDurationSection2 = 0.6528; %variable in future but for now fixed

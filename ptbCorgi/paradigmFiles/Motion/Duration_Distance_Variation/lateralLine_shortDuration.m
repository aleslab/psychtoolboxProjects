function [conditionInfo, expInfo] = lateralLine_shortDuration(expInfo)

%paradigm for exp 6 (experiment investigating the role of distance and 
%duration in speed discrimination).

expInfo.paradigmName = 'lateralLine_shortDuration';
...
%for iGap = 1:length(spatialGap);
    ...
    for iSpeed = 1: length(section2velocity);
        ...
        conditionInfo(iCond).label = ['short_dur_second_speed_' num2str(section2velocity(iSpeed))];
        ...
        conditionInfo(iCond).stimDurationSection2 = 0.38296; %variable in future but for now fixed

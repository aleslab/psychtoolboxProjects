function [conditionInfo, expInfo] = lateralLine_flipped_gap_screening(expInfo)

%screening paradigm for exp 4 (temporal integration experiment).

expInfo.paradigmName = 'lateralLine_flipped_gap_screening';
...
%velocities
section2velocity = [5 15]; %velocities to use in deg/s;
...
for iGap = 1:length(temporalGap);
    ...
    for iSpeed = 1: length(section2velocity);
        ...
        conditionInfo(iCond).stimDurationSection1 = 0.20; %approximate stimulus duration in seconds
        ...
        conditionInfo(iCond).startPos = -4.75; %the start position of the line on the screen in degrees of visual angle.

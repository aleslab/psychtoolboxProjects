function [conditionInfo, expInfo] = psychParadigm_MAE2(expInfo)
...
conditionInfo(1).trialFun=@trial_MAE2;
...
conditionInfo(1).vblAdaptTopUP = 1; % re-adaptation 10 seconds
conditionInfo(1).adaptDuration = 5; % % Adaptation duration 30 s
conditionInfo(1).nbRepeat = 8; % nb adaptation in addition to the first adaptation = 8
conditionInfo(1).testDuration = 1; % in cycles. 5 seconds = 20/85*20 cycles (exactly 4.7 seconds)

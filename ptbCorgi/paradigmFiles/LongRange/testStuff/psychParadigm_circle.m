function [conditionInfo, expInfo] = psychParadigm_circle(expInfo)
...
%% stimulus
conditionInfo(1).stimSize = [0 0 1.5 1.5]; % in deg
...
conditionInfo(1).trialFun=@trial_circle;
conditionInfo(1).trialDuration = 4*6*32/85; % 6*32/85; % in sec - around 9.0353 (or 100*8/85 or 50*16/85)
...
testedFreq = [85/16 85/32]; % in Hz this is the onset of the single stimulus
onTime = [1 2 4 6 7];
stim = 1:3;
...
for testFq=1:length(testedFreq)
    for tt = 1:length(onTime)
        ...
        conditionInfo(condNb).stimTagFreq = testedFreq(testFq); 
        conditionInfo(condNb).dutyCycle = onTime(tt)/8;
        conditionInfo(condNb).stim = stim(nn);

function [conditionInfo, expInfo] = psychParadigm_Ecc(expInfo)
...
conditionInfo(1).trialFun=@trial_Ecc;
...
for cc=2:length(testedFreq)*length(onTime)*length(motionType)*length(testEcc)
    conditionInfo(cc) = conditionInfo(1);
end

condNb = 1;
for ecc=1:length(testEcc)
    for mot=1:length(motionType)
        for testFq=1:length(testedFreq)
            for tt = 1:length(onTime)
%                 for d0.6irection=1:2
%                     for order=1:2
                        conditionInfo(condNb).xloc = testEcc(ecc);
...
%                         conditionInfo(condNb).order = order;
                        conditionInfo(condNb).label = ['E' num2str(ecc) 'm' num2str(mot) '_' num2str(round(testedFreq(testFq))) 'Hz ' num2str(round(onTime(tt)/8*100)) '%'];
                        condNb = condNb+1;
%                     end
%                 end
            end

...
expInfo.fixationInfo(1).loc = [0 3]; % location of the fixation relative to centre in degrees (1st number is vertical, 2nd is horizontal)
...
%% stimulus
conditionInfo(1).stimSize = [0 0 1 15]; % in deg
...
conditionInfo(1).stimTagFreq = 85/16;
...
for cc=5:8
    conditionInfo(cc).xloc = [-3 -1 1 3]; % eccentricity of stim centre from screen centre in deg
end

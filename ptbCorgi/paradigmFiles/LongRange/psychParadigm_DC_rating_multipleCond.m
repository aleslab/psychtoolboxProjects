function [conditionInfo, expInfo] = psychParadigm_DC_rating_multipleCond(expInfo)
...
conditionInfo(1).trialFun=@trial_DC_rating_multipleCond;
...
for cc=2:length(motion)*length(testedFreq)*length(onTime)
...
for testFq=1:length(testedFreq)
    for tt = 1:length(onTime)
        ...
        conditionInfo(condNb).label = ['M' num2str(motion(mm)) 'F' num2str(round(testedFreq(testFq))) 'DC' num2str(round(onTime(tt)/8*100))];
        condNb = condNb+1;
    end
end
end

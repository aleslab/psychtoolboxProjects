function [conditionInfo, expInfo] = psychParadigm_dutyCycle_lessConditions(expInfo)

KbName('UnifyKeyNames');

%paradigmName is what will be prepended to data files
expInfo.paradigmName = 'dutyCycle_lessConditions';
...
%% stimulus
conditionInfo(1).stimSize = [0 0 0.5 15]; % in deg
...
conditionInfo(1).trialDuration = 10*32/85; % 5*5*32/85; % in sec - around 9.4118 (or 100*8/85 or 50*16/85)
conditionInfo(1).trialFun=@trial_dutyCycle_lessConditions;
conditionInfo(1).motion = 0; % by default, no motion 
conditionInfo(1).dotSize = [0 0 0.2 0.2];
conditionInfo(1).preStimDuration = 0; % 3*32/85; % around 1.1294 s (or 6*16/85, 12*8/85)
...
testedFreq = 85/16; % in Hz this is the onset of the single stimulus
onTime = [2 6];
for tt = 1:length(onTime)
    conditionInfo(condNb).stimTagFreq = testedFreq;
    conditionInfo(condNb).dutyCycle = onTime(tt)/8;
    conditionInfo(condNb).motion = 1;
    conditionInfo(condNb).label = ['motion ' num2str(testedFreq) 'Hz ' num2str(onTime(tt)/8*100) '% DC'];
    condNb = condNb+1;
end

function [conditionInfo, expInfo] = psychParadigm_multipleFreq(expInfo)
% One Bar stim tested on multiple freq
% trial duration min 6 cycles (for the 2 slowest freq) or 2.26 sec (for the
% fastest freq)
...
expInfo.fixationInfo(1).loc = [-4 0]; % location of the fixation relative to centre in degrees (1st number is horizontal, 2nd is vertical)
...
conditionInfo(1).xloc = 6; % eccentricity of stim centre from screen centre in deg
conditionInfo(1).yloc = 0; % y eccentricity of stim centre
conditionInfo(1).trialFun=@trial_multipleFreq;
conditionInfo(1).trialDurationMax = 6*32/85; % in sec - around 9.0353 (or 100*8/85 or 50*16/85)
...
testedFreq = [85/128 85/64 85/32 85/24 85/16 85/8]; % in Hz this is the onset of the single stimulus
onTime = 1:7;

function [trialData] = trial_dutyCycle_lessConditions(expInfo, conditionInfo)
...
for cycleNb = 1 : nbTotalCycles
    ...
    if conditionInfo.motion == 1 && mod(cycleNb,2)==0 % in motion
        ...
        Screen('FillRect', expInfo.curWindow, black,CenterRectOnPoint(rectCircle,eccMotion+loc1,ycoord));
        Screen('FillRect', expInfo.curWindow, black,CenterRectOnPoint(rectCircle,eccMotion+loc2,ycoord));
        ...
        Screen('FillRect', expInfo.curWindow, black,CenterRectOnPoint(rectCircle,xcoord+loc1,ycoord));
        Screen('FillRect', expInfo.curWindow, black,CenterRectOnPoint(rectCircle,xcoord+loc2,ycoord));
        if ismember(cycleNb,trialData.dots) % check for stim to detect
            yDot = (maxYdot-minYdot)*rand(1)+minYdot;

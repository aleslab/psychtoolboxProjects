function [trialData] = trial_diffratings(expInfo, conditionInfo)
...
if trialData.validTrial
    ...
    Screen('DrawText', expInfo.curWindow, 'none: 2', 0, expInfo.center(2)+expInfo.center(2)/8, [0 0 0]);
    Screen('DrawText', expInfo.curWindow, 'left: 4', 0, expInfo.center(2)+expInfo.center(2)*2/8, [0 0 0]);
    Screen('DrawText', expInfo.curWindow, 'right: 6', 0, expInfo.center(2)+expInfo.center(2)*3/8, [0 0 0]);
    Screen('DrawText', expInfo.curWindow, 'both: 8', 0, expInfo.center(2)+expInfo.center(2)*4/8, [0 0 0]);        
    ...
    while trialData.response==999 % && (GetSecs < trialData.respScreenTime + conditionInfo.maxToAnswer -ifi/2)
        ...
        if keyDown
            if length(find(keyCode)) == 1 % only one key pressed
                    if strcmp(KbName(keyCode),'2')  || strcmp(KbName(keyCode),'4') || strcmp(KbName(keyCode),'6') || strcmp(KbName(keyCode),'8')

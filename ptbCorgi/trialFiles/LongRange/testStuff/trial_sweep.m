function [trialData] = trial_sweep(expInfo, conditionInfo)
...
stimStartTime = 0;
gray = GrayIndex(expInfo.curWindow); % colour for screen lines used for triggers
...
Screen('FillRect', expInfo.curWindow, gray,[0 0 1280 3]);
...
xcoord1 = expInfo.center(1) + (conditionInfo.xloc * expInfo.ppd);
xcoord2 = xcoord1 + (conditionInfo.xMotion * expInfo.ppd); 
% random first position
firstX = shuffle([xcoord1 xcoord2]);
xcoord = firstX(1);
eccMotion = firstX(2);
...
for cycleNb = 1 : nbTotalCycles
    ...
    if conditionInfo.motion == 1 && mod(cycleNb,2)==1 % in motion
        Screen('FillRect', expInfo.curWindow, conditionInfo.stimColour(cycleNb),CenterRectOnPoint(rectStim,eccMotion,ycoord));
    ...
    elseif conditionInfo.motion == 0
        Screen('FillRect', expInfo.curWindow, conditionInfo.stimColour(cycleNb),CenterRectOnPoint(rectStim,xcoord,ycoord));
        Screen('FillRect', expInfo.curWindow, conditionInfo.stimColour(cycleNb),CenterRectOnPoint(rectStim,eccMotion,ycoord));

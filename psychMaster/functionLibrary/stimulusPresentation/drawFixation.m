...
switch lower(fixationInfo.type)
    ...
    case 'noiseframe'
        ...
        if ~isfield(fixationInfo,'rectSize') || isempty(fixationInfo.rectSize)
            fixationInfo.rectSize = 100;
        end
        ...
        if ~isfield(expInfo,'fixationTextures') || isempty(expInfo.fixationTextures)
            ...
            leftRectMat = randn([screenYpixels, fixationInfo.rectSize]);
            rightRectMat = randn([screenYpixels, fixationInfo.rectSize]);
            topHorzMat = randn([fixationInfo.rectSize, screenXpixels]);
            bottomHorzMat = randn([fixationInfo.rectSize, screenXpixels]);
        ...
        leftRectLocation = [0 0 fixationInfo.rectSize screenYpixels];
        rightRectLocation = [(screenXpixels-fixationInfo.rectSize) 0 screenXpixels screenYpixels];
        topRectLocation = [0 0 screenXpixels fixationInfo.rectSize]; %this draws the top rectangle along the entire length of the top of the screen
        bottomRectLocation = [0 (screenYpixels-fixationInfo.rectSize) screenXpixels screenYpixels];

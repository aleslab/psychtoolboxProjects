function [conditionInfo, expInfo] = psychParadigm_MoveLine_2afc_combined_slow_retinal_speed(expInfo)
...
expInfo.paradigmName = 'MoveLine_combined_constant_retinal_speed_slow';

%% conditions
firstVelocities = [0.629 0.643 0.725 0.810 0.894 0.984 1.072];
...
conditionInfo(iCond).velocityCmPerSecSection2 = (1.258)-(conditionInfo(iCond).velocityCmPerSecSection1); %cm/s 
conditionInfo(iCond).isNullCorrect = false;
conditionInfo(iCond).objectOneStartPos = -1.3145; %when there are two lines in each eye, the start position of the first line
conditionInfo(iCond).objectTwoStartPos = 0.6855; %the start position of the second line in each eye
...
nullCondition.velocityCmPerSecSection1 = 0.629;  
nullCondition.velocityCmPerSecSection2 = 0.629;

...
%% conditions
firstVelocities = repmat([-20, 20],4); % first section velocities are all 20cms-1 but in different directions
condStimTypes = cat(2, repmat( {'combined'},1,4) , repmat( {'lateralCombined'},1,4)); %condition types cd cd lateralCd lateralCd
stimDuration1 = repmat([0.25 0.25 0.5 0.5],2);
stimDuration2 = stimDuration1;
depthStart = repmat([5 -5 10 -10],2);
